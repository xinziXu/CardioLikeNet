function [qrs_amp_raw,qrs_i_raw,delay,ecg_h,ecg_m,locs,SIG_buf,SIG1_buf,pole_buf]=pan_tompkin(ecg,fs,gr)
...
%        ecg_l(n)=ecg(n);
%    elseif n<=2
%        ecg_l(n)=2*ecg_l(n-1)+ecg(n);
%    elseif n<=6
%        ecg_l(n)=2*ecg_l(n-1)-ecg_l(n-2)+ecg(n);
%    elseif n<=12
%        ecg_l(n)=2*ecg_l(n-1)-ecg_l(n-2)+ecg(n)-2*ecg(n-6);
%    else
%        ecg_l(n)=2*ecg_l(n-1)-ecg_l(n-2)+ecg(n)-2*ecg(n-6)+ecg(n-12);
%    end
% end
% ecg_h = ecg_l;
...
ecg_h  = MedianFilt(ecg_l,round(0.25*fs));
if gr
...
% for n=1:length(ecg_h)
%     if n<=1
%         ecg_d(n)=2*ecg_h(n+1)+ecg_h(n+2);
%     elseif n<=2
%         ecg_d(n)=2*ecg_h(n+1)+ecg_h(n+2)-2*ecg_h(n-1);
%     elseif n<=length(ecg_h)-2
%         ecg_d(n)=2*ecg_h(n+1)+ecg_h(n+2)-2*ecg_h(n-1)-ecg_h(n-2);
%     elseif n<=length(ecg_h)-1
%         ecg_d(n)=2*ecg_h(n+1)-2*ecg_h(n-1)-ecg_h(n-2);
%     else
%         ecg_d(n)=-2*ecg_h(n-1)-ecg_h(n-2);
%     end
% end
...
%  for n=32:length(ecg_s)
%      ecg_m(n)=sum(ecg_s((n-31):n))/32;
%  end
%  ecg_m = ecg_m';
ecg_m = conv(ecg_s ,ones(1 ,32)/32);
delay = delay + 32;

if gr
ax(6)=subplot(326);plot(ecg_m);axis tight;title('Averaged with 32 samples length,Black noise,Green Adaptive Threshold,RED Sig Level,Red circles QRS adaptive threshold');
axis tight;
end
...
THR_SIG = max(ecg_m(4*fs:6*fs))*1/4;
THR_SIG1 = max(ecg_h(4*fs:6*fs))*1/4; % 0.25 of the max amplitude 
baseline = 0.25*mean(ecg_h(4*fs:6*fs));
THR_SIG_neg= 0;
qrs_i = [];
qrs_a = [];
qrs_amp_raw=[];
...
for i=1:length(pks)
    if (length(qrs_i)>8)
        ...
        if (locs(i)-qrs_i(end))<= 0.5*mean_RR % rr interval is too short, tell it is T wave or not
            
            if (locs(i)-qrs_i(end))>0.3*fs
            if pks(i)>THR_SIG

%                 if -10<(ecg_m(locs(i))-ecg_m(qrs_i(end)))<10
%                 Slope1 = mean(diff(ecg_m(locs(i)-round(0.075*fs):locs(i))));
%                 Slope2 = mean(diff(ecg_m(qrs_i(end)-round(0.075*fs):qrs_i(end))));
%                 if abs(Slope1) >= abs(0.75*(Slope2)) 
%                     qrs_i = [qrs_i locs(i)];
%                     qrs_a = [qrs_a pks(i)];
                    THR_SIG = 0.25*pks(i)+0.5*THR_SIG;
                    
                    [y_i x_i pole]=find_rpeak(ecg_h,locs(i),fs,THR_SIG1,THR_SIG_neg);
                    if pole == 1
                        if y_i>THR_SIG1
                          qrs_i_raw = [qrs_i_raw x_i];% save index of bandpass 
                          qrs_amp_raw =[qrs_amp_raw y_i]; %save amplitude of bandpass 
                          THR_SIG1 = 0.25*y_i+0.5*THR_SIG1; %when found with the second thres 
                          SIG1_buf = [SIG1_buf THR_SIG1];
                          pole_buf = [pole_buf 1];
                        end
                    else
                        if y_i <THR_SIG_neg
                          qrs_i_raw = [qrs_i_raw x_i];% save index of bandpass 
                          qrs_amp_raw =[qrs_amp_raw y_i]; %save amplitude of bandpass 
                          THR_SIG_neg = 0.25*y_i+0.5*THR_SIG_neg; %when found with the second thres 
                          SIG1_buf = [SIG1_buf THR_SIG_neg];
                          pole_buf = [pole_buf -1];
                        end
                    end
%                 end
        ...
        elseif (locs(i)-qrs_i(end))>=1.6*mean_RR%感觉要miss一个r峰，所以往回找峰值
%             length(qrs_i)

            [pks_temp,locs_temp] = max(ecg_m(qrs_i(end)+ round(0.25*fs):locs(i)-round(0.2*fs)));
            locs_temp = qrs_i(end)+ round(0.200*fs) + locs_temp -1  ;

            if  ((pks_temp> 0.14*THR_SIG) && ((locs_temp-qrs_i(end))>0.25*mean_RR))
                    qrs_a = [qrs_a pks_temp];
                    qrs_i = [qrs_i locs_temp];
                    THR_SIG = 0.25*pks_temp+0.25*THR_SIG;
%                     SIG_buf = [SIG_buf THR_SIG];

                    THR_SIG1 = 0.2*THR_SIG1;
                    THR_SIG_neg = 0.2*THR_SIG_neg;
                    [y_i_t x_i_t pole]=find_rpeak(ecg_h,locs_temp,fs,THR_SIG1,THR_SIG_neg);
                    Slope1 = mean(diff(ecg_h(x_i_t-round(0.02*fs):x_i_t)));
                    Slope2 = mean(diff(ecg_h(qrs_i_raw(end)-round(0.02*fs):qrs_i_raw(end))));
                    if  abs(Slope1)>3 || abs(Slope1) >= abs(0.8*(Slope2)) 
%                     if pole == 1
%                     y_i_t 
%                     length(qrs_i_raw)
%                     if y_i_t > 0.5*THR_SIG1 
                      qrs_i_raw = [qrs_i_raw x_i_t];% save index of bandpass 
                      qrs_amp_raw =[qrs_amp_raw y_i_t]; %save amplitude of bandpass 
                      THR_SIG1 = 0.25*y_i_t+0.5*THR_SIG1; %when found with the second thres   
                      SIG1_buf = [SIG1_buf THR_SIG1];
                      pole_buf = [pole_buf pole];
                    end
                    end
%                     else
%                         if y_i_t < 0.5*THR_SIG_neg
%                             qrs_i_raw = [qrs_i_raw locs_temp-round(0.150*fs)+ (x_i_t - 1)];% save index of bandpass 
%                             qrs_amp_raw =[qrs_amp_raw y_i_t]; %save amplitude of bandpass 
%                             THR_SIG_neg = 0.25*y_i_t+0.25*THR_SIG_neg; %when found with the second thres
%                             SIG1_buf = [SIG1_buf THR_SIG_neg];
%                             pole_buf = [pole_buf pole];
%                         end
%                     end
            ...
            if pks(i)>THR_SIG
                ...
                if (locs(i)-qrs_i(end))>0.25*fs


                 qrs_i = [qrs_i locs(i)];
                 qrs_a = [qrs_a pks(i)];
                 THR_SIG = max(40,0.25*pks(i)+0.25*THR_SIG);
%                  SIG_buf = [SIG_buf THR_SIG];
...
%             length(qrs_i)
            if pks(i)>THR_SIG
                ...
                if (locs(i)-qrs_i(end))>0.25*fs
                 ...
                 qrs_i = [qrs_i locs(i)];
                 qrs_a = [qrs_a pks(i)];
                 THR_SIG = 0.25*pks(i)+0.3*THR_SIG;
%                  SIG_buf = [SIG_buf THR_SIG];
                 [y_i x_i pole]=find_rpeak(ecg_h,locs(i),fs,THR_SIG1,THR_SIG_neg);
...
%                 end
        ...
        if pks(i)>THR_SIG
              ...
              qrs_a = [qrs_a pks(i)];
%               SIG_buf = [SIG_buf THR_SIG];
%               THR_SIG = 0.25*pks(i)+0.25*THR_SIG;
              [y_i x_i pole]=find_rpeak(ecg_h,locs(i),fs,THR_SIG1,THR_SIG_neg);
...
%               if y_i > THR_SIG1
%                   
%                   qrs_i_raw = [qrs_i_raw locs(i)-round(0.150*fs)+ (x_i - 1)];
%                   qrs_amp_raw =[qrs_amp_raw y_i];
%                   SIG1_buf = [SIG1_buf THR_SIG1];
%                   pole_buf = [pole_buf 1];
% %                   THR_SIG1 = 0.25*y_i+0.25*THR_SIG1;
%               end
        end
    end
SIG_buf = [SIG_buf THR_SIG];            
...
%                
%                not_nois = 1;
%                SIG_LEV = 0.25*pks_temp + 0.25*SIG_LEV ;  %when found with the second threshold             
%              end 
%               
%           else
%               not_nois = 0;
